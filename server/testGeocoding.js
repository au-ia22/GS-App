...
async function testGeocoding() {
  ...
  for (const record of records) {
    ...
    projectMap.set(projectId, { displayName, fullAddress });
  }

  console.log(`Testing geocoding for ${projectMap.size} projects...\n`);
  ...
  for (const [projectId, projectData] of projectMap) {
    const result = await geocodeAddress(projectData.fullAddress);
    ...
    if (result.success) {
      ...
      failCount++;
      console.log(`✗ ${projectId}: ${result.error}`);
      failures.push({ projectId, fullAddress: projectData.fullAddress, error: result.error });
    }
  ...
  if (failures.length > 0) {
    console.log(`\n=== FAILED PROJECTS ===`);
    failures.forEach(f => {
      console.log(`${f.projectId} | ${f.fullAddress} | ${f.error}`);
    });
  }
